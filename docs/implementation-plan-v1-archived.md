...
> **本文档已归档**。架构方向已在 v2 中重定。
>
> **废弃原因**：v1 假设「整站用 BangNiCMS 主题+插件搭，作为 dogfooding 演示」，但与用户实际需求不符。用户的真实需求是：
>
> 1. **首页**：讲清楚 BangNiCMS 的价值（静态即可）
> 2. **文档**：标准三栏文档站（左目录/中正文/右页内锚点）
> 3. **扩展市场**：发布扩展包，让用户的 BangNiCMS 后台能拉到
>
> 这三件事**完全不需要部署 BangNiCMS 实例**，用单一 Astro+Starlight 静态站即可。v2 把工程量从 12 周缩到 4-5 周。
>
> **请阅读最新方案**：[`implementation-plan-v2.md`](./implementation-plan-v2.md)
>
> 本文档保留作为历史决策记录，不再维护。
